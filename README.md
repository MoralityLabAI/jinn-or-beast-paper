...
| Program | Location | Status (2026-07-19) |
|---|---|---|
...
| exp:3 frame internalization SFT | `ConstitutionalAlignment/experiments/frame_internalization_sft_v1/` | CENTRAL EXPERIMENT. Current upstream `de2853f`; readiness receipt anchored at `a841e5a`: **5 gates passed, 6 compute-blocking gates pending**, `pilot_ready: false`, and no fine-tuning outcome. The direct prompt-versus-SFT `D_f` contract passes. Historical compact summaries and artifact locators are recovered, but the registered row/activation bytes remain unavailable/unverified. Gate 1 cluster-cache verification, curricula, final nonleakage, predecessor reanchor, distributed smoke, and authorization remain pending. See the [execution checklist](protocols/experiment_execution_checklist_v1.md). |
| Supplemental Mīzān/Bonsai prompt sensitivity | `ConstitutionalAlignment/experiments/mizan_rooms_v1/` | Prospective v2 execution freeze at `de2853f`: exact Bonsai GGUF-tokenizer cue audit passed. The v1 local launch was cancelled before any prompt after detecting a cross-arm treatment disclaimer; zero development and zero evaluation responses exist. V2 development smoke and the 900-turn exploratory matrix remain unrun. |
