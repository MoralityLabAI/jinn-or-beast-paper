...
| `experiments/` | Registered local experiments, prepared task decks, outputs, and run receipts. |
| `skills/` | Reusable Jinn/Beast persona and typed-membrane agent control flow. |
...
| Exogenous Jinn/Beast skill + MeTTa membrane | `experiments/exogenous_skill_membrane_v1/` | **CENTRAL COMPLETED RESULT.** Prospective implementation frozen at `76b0af3`; 96/96 held-out Qwen3-1.7B traces completed under hard caps. Persona skills changed 4/12 Jinn and 5/12 Beast model-only actions. Matched membrane: one targeted successful correction, zero critical finals. Shuffled membrane: 11 revisions and three critical finals. This supports selective system-level control, not adapter internalization or validated theology. |
| exp:3 frame internalization SFT | `ConstitutionalAlignment/experiments/frame_internalization_sft_v1/` | REGISTERED FOLLOW-UP, not the current behavioral result. Current upstream `a91342d`. Because Silico and its INTELLECT-3 cache are unavailable, registered execution was prospectively retargeted—before any registered Qwen outcome—to official `Qwen/Qwen3-1.7B@70d244c`. The six-arm, 5,600-dilemma, 4,096-token study remains unrun and `pilot_ready: false`. Separately, a prospectively frozen no-cloud 30-step MeTTa-derived 1.7B screen completed on the local RTX 3050: infrastructure passed, but the flavored and guided worldview screens did not. That negative recipe-level result does not replace the registered study, veto a later 4B test, or authorize 4B/9B spend. See the [execution checklist](protocols/experiment_execution_checklist_v1.md). |
...
| Two-frame MeTTa LDT data sorter | `experiments/two_frame_metta_ldt_v1/` | COMPLETE bounded CPU policy-emulation run. Jinn and Beast heads reached 96.30% and 97.92% test accuracy with 99.07% and 99.77% candidate coverage. Eleven of twelve preregistered checks passed; Beast opposite-frame invariance was 94.91% against a 95% gate, so `pilot_ready: false`. Exact audited-tag routes are materialized; raw text remains review-only. |
...
3. The paper tests theological frames as **operational interventions**, never
